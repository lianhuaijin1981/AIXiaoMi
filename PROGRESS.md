...
## 2026-05-10 - Three.js 3D 模型渲染增强（迭代 6）

### 本次完成功能
1. **✅ 3D 模型操作 UI 控制面板**
   - 添加旋转控制按钮（X/Y/Z 轴，±15°）
   - 添加缩放控制按钮（放大 1.2x / 缩小 0.8x / 重置）
   - 添加平移控制按钮（上/下/左/右）
   - 添加重置视图按钮（重置模型位置、旋转、缩放）
   - 添加自动旋转开关
   - 实时显示当前旋转角度
...
2. **✅ 模型材质和纹理支持**
   - 添加材质编辑面板（可折叠）
   - 支持颜色选择（color picker）
   - 支持金属度调整（metalness: 0-1）
   - 支持粗糙度调整（roughness: 0-1）
   - 支持线框模式切换（wireframe）
   - 实时更新模型材质属性
   - 支持遍历模型所有 Mesh 并更新材质
...
3. **✅ 模型快照功能**
   - 添加"快照"按钮
   - 将当前 3D 视图渲染为 PNG 图片
   - 自动下载 PNG 文件（文件名包含时间戳）
   - 支持白色背景快照

4. **✅ 优化用户体验**
   - 添加顶部工具栏（控制面板、材质面板、快照按钮）
   - 控制面板可折叠
   - 材质面板可折叠
   - 优化布局（控制面板在右侧，材质面板在控制面板左侧）

### 技术要点
- **UI 控制面板**：
  - 使用 `useCallback` 优化性能
  - 旋转：修改 `model.rotation`（弧度）
  - 缩放：使用 `model.scale.multiplyScalar(factor)`
  - 平移：修改 `model.position`
  - 重置：恢复默认值并重新计算相机位置
- **材质编辑**：
  - 遍历模型所有 `Mesh` 对象
  - 支持 `MeshStandardMaterial` 属性修改
  - 支持数组材质（`child.material.forEach`）
  - 实时更新：`React.setState` + Three.js 材质属性
- **快照功能**：
  - 使用 `renderer.domElement.toDataURL('image/png')` 获取 PNG 数据
  - 临时修改背景色为白色
  - 渲染一帧后恢复原始背景
  - 创建 `<a>` 标签触发下载
- **自动旋转**：
  - 在动画循环中更新 `model.rotation.y += 0.01`
  - 使用 `useState` 控制开关

### 文件变更清单
#### 修改文件
1. `src/components/ThreeDViewer.tsx` - 添加 UI 控制面板、材质编辑、快照功能

### Git 提交记录
```
28f3378 - feat: 集成 Three.js 实现真实 3D 模型渲染
```
